...
public class HW4Tests {
	...
	/**
	 * This test creates deadlock, then checks to see three things, increasing in
	 * order of strictness:
	 * 1. That at least one lock has survived deadlock
	 * 2. That whichever transaction survived deadlock still has both of its locks
	 * (This is to ensure that locks are rolled back by transaction, not
	 * individually)
	 * 3. That the deadlock is truly resolved, and there are no conflicting locks
	 * left
	 * 
	 * @throws Exception
	 */
